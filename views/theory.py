...
def view(game):
    ...
    def set_theory_page(page_no):
        ...
        if vw(100) / vh(100) > theory_rect.width / theory_rect.height:
            theory_page = utils.scale_image_maintain_ratio(theory_page, h = vh(100))
        else:
            theory_page = utils.scale_image_maintain_ratio(theory_page, w = vw(100))
        theory_content["page_surf"] = theory_page
        theory_content["current_page"] = page_no

    prev_btn = Button(vw(10), vh(90), "Previous", w=vw(18), h = vh(7))
    prev_btn.on_click = lambda : set_theory_page(theory_content["current_page"] - 1)
    next_btn = Button(vw(80), vh(90), "Next", w=vw(18), h = vh(7))
    next_btn.on_click = lambda : set_theory_page(theory_content["current_page"] + 1)
    close_btn = Button(vw(80), vh(90), "Close", w=vw(18), h = vh(7))
