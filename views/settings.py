...
def view(game):
    ...
    difficulty_text = config.font.lg.render("Difficulty",
                                            True,
                                            config.front_color)
    difficulty_btn = Button(vw(44), vh(24),
                            DIFFICULTY_OPTIONS[config.difficulty],
                            w=vw(12), h=vh(5),
                            font=config.font.md)
    ...
    def robot_switch():
        nonlocal robot_btn
        current_robot_index = ROBOTS.index(config.current_robot)
        config.current_robot = ROBOTS[(current_robot_index + 1) % len(ROBOTS)]

        robot_btn = Button(vw(10), vh(40), "",
                           w=vw(30), h=vh(30),
                           font=config.font.md)
        robot_btn_surf = config.images["robot"][config.current_robot]
        robot_btn_surf = utils.scale_image_maintain_ratio(robot_btn_surf,
                                                          h=vh(30))
    ...
    def object_switch():
        nonlocal object_btn
        current_index = OBJECTS.index(config.current_object)
        config.current_object = OBJECTS[(current_index + 1) % len(OBJECTS)]

        object_btn = Button(vw(71), vh(43), "",
                            w=vw(30), h=vh(30),
                            font=config.font.md)
        object_btn_surf = config.images["object"][config.current_object]
        object_btn_surf = utils.scale_image_maintain_ratio(object_btn_surf,
                                                           h=vh(25))
